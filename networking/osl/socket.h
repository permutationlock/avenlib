...
/** Receive these bytes and from address from this socket.  Returns 0 on success;
  else calls abort routine.
*/
int skt_recvN_from(SOCKET skt, void *pBuff, int nBytes, sockaddr *from, unsigned int *fromlen);

/** Send these bytes to this socket to the designated address.  Returns 0 on success;
  else calls abort routine.
*/
int skt_sendN_to(SOCKET skt, const void *pBuff, int nBytes, const sockaddr *to, unsigned int tolen);
