...
int skt_recvN_from(SOCKET hSocket, void *buff, int nBytes, sockaddr *from, unsigned int *fromlen)
{
  int nLeft, nRead;
  ...
  while (0 < nLeft)
  {
    ...
    skt_ignore_SIGPIPE=1;
    nRead = recvfrom(hSocket,pBuff,nLeft,0,from,fromlen);
    skt_ignore_SIGPIPE=0;
    if (nRead<=0)
    {
...
int skt_sendN_to(SOCKET hSocket, const void *buff, int nBytes, const sockaddr *to, unsigned int tolen)
{
  int nLeft, nWritten;
  ...
  {
    skt_ignore_SIGPIPE=1;
    nWritten = sendto(hSocket,pBuff,nLeft,0,to,tolen);
    skt_ignore_SIGPIPE=0;
    if (nWritten<=0)
    {
